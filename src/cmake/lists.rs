...
pub fn add_to_target_sources(path: &str, target_name: &str, new_source_file_path: &str) {
	...
	//FIXME: check what error we actaully get
	let contents = fs::read_to_string(cmake_path)
		.expect("Error Reading CMake File");
	...
	for line in content_lines {
		...
		if stripped_line.starts_with(&sources_string) {
			...
			let new_start = format!("{} \r\n\tPUBLIC {}\r\n", sources_string, new_source_file_path); //FIXME: Need to have an option for interface, public, or private
	...
	fs::write(cmake_path, new_content_lines)
