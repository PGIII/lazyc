...
fn create_module_files(path: &str, module_name: &str) {
    ...
    let mut src_file =
        File::create(Path::new(&source_path)).expect("Error Creating Module Source File");
    let mut header_file =
        File::create(Path::new(&header_path)).expect("Error Creating Module Header File");
    ...
    src_file
        .write_all(src_temp_str.as_bytes())
        .expect("Error Writing Module Src");
    header_file
        .write_all(header_temp_str.as_bytes())
